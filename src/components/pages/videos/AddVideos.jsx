...
export default function AddVideos() {
    ...
    let [categoryStatus, setCategoryStatus] = useState(true)
    ...
    useEffect(() => {
        if (params.video_id != undefined) {
            axios.post('http://localhost:8000/api/backend/videos/detail/' + params.video_id).then((result) => {
                axios.post('http://localhost:8000/api/backend/videos/edit-course-category/' + params.category_id).then((success) => {
                    // console.log(success.data.data.status)
                    if (success.data.data.status == true) {
                        setInput({
                        ...
                        setInput({
                            video_category: 'This Course Category Is Deactivated',
                            video_topic: result.data.data.topic,
                            video_link: result.data.data.link,
                            video_status: result.data.data.status
                        })
                        setCategoryStatus(success.data.data.status)
                    }
                }).catch((error) => {
                                ...
                                <div className='w-70 mx-auto p-5 shadow-lg p-3 bg-body rounded border'>
                                    ...
                                    <form onSubmit={saveFormeData}>
                                        <div class="mb-3">
                                                    ...
                                                    fetchCourseCategory.map((courseCategory, index) => (
                                                        <option key={index} value={courseCategory._id} onChange={inputHandeler} selected={(input.video_category == courseCategory._id)? 'selected' : '' }>
                                                            {(courseCategory.status) ? courseCategory.name : "This Course Category Is Deactivated"}
                                                             ...
                                                             {courseCategory.name}
                                                        {(categoryStatus) ? courseCategory.name : "This Course Category Is Deactivated"}
                                            ...
                                            <select name='video_category' className="form-control mb-3" onChange={inputHandeler}> <option>Select video category</option>
                                                {fetchCourseCategory.length > 0 ? (fetchCourseCategory.map((courseCategory, index) => (<option key={index} value={courseCategory._id} selected={input.video_category === courseCategory._id ? true : false} > {courseCategory.name} </option>))) : (<option>No categories available</option>)} 
                                                {/* If categoryStatus is false, add a disabled option indicating deactivation */} {!categoryStatus && (<option value="" disabled selected> This Course Category Is Deactivated </option>)} </select>
                                        </div>
                                        ...
                                        <input type="submit" className="btn btn-primary fw-bold " value={(params.video_id == undefined) ? 'Submit' : 'Update'} />
